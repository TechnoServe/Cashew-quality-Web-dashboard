...
class FreeQar extends \common\models\FreeQar
{
    ...
    public static function getKorsAndLocations($startDate, $endDate, $locationFilters = [], $rtn = "list")
    {
        $q = self::find()
            ->innerJoin("free_qar_result", "free_qar_result.qar = free_qar.document_id")
            ->leftJoin("free_sites", "free_sites.document_id = free_qar.site")
        ...
        switch ($rtn){
            case "average" :
                return $q->average("free_qar_result.kor");
                break;
            case "sum" :
                return $q->sum("free_qar_result.kor");
                break;
            default:
